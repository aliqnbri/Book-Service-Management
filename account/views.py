...
class UserViewSet(viewsets.ViewSet):
    ...
    def login(self, request):
        serializer = self.get_serializer(data=request.data)
        serializer.is_valid(raise_exception=True)
        dict_instance = User.get(
            username=serializer.validated_data['username'])[0]
        access_toekn, refresh_token = JWTService.token_generator(
            dict_instance), JWTService.refresh_token_generator(dict_instance)
    ...
    def retrieve(self, request, id: int) -> Response:
        ...
        serializer = self.get_serializer(
            user_data, context={'request': request, 'view': self})
    ...
    def suggest(self, request, id=None):
        ...
        serializer = self.get_serializer(recommended_books, many=True, context={
                                         'request': request, 'view': self})
        return Response(serializer.data, status=status.HTTP_200_OK)
...
class ReviewViewSet(viewsets.ViewSet):
    ...
    def retrieve(self, request, id=None, **kwargs):
        ...
        serializer = self.serializer_class(
            review[0], context={'request': request, 'view': self})
        return Response(serializer.data, status=status.HTTP_200_OK)
    ...
    def update(self, request, user_id=None, id=None):
        ...
        if not review:
            return Response({'error': 'Review not found'}, status=status.HTTP_404_NOT_FOUND)
        serializer = self.serializer_class(review, data=request.data, context={
                                           'request': request, 'view': self})
        serializer.is_valid(raise_exception=True)
        validated_data = serializer.validated_data
